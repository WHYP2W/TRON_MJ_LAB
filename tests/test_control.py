...
def make_action(**overrides):
    ...
    return UpperBodyAction(
        UpperBodyActionCfg(entity_name="robot", **overrides), env
    )
...
def test_control_configuration_requires_finite_positive_values(
    ...
    with pytest.raises(ValueError, match=parameter):
        make_action(**{parameter: value})


def test_upper_body_has_no_policy_actions():
    term = make_action()
    assert term.action_dim == 0
    assert term.raw_action.shape == (3, 0)
